...
void processOutLinkSLRA(Cluster inCluster[3], tower_t inTower[16], ap_uint<576> &outLink){
...
clusterInfo cluster_tmp;
       ...
       for(loop i=0; i<CRYSTAL_IN_ETA; i++){
        ...
        for(loop m=0; m<3 ; m++){
       	   #pragma HLS UNROLL
        	tmp1 		 = temp[seed_eta1+m][seed_phi1+0] + temp[seed_eta1+m][seed_phi1+1];
        	tmp2 	     = temp[seed_eta1+m][seed_phi1+2] + temp[seed_eta1+m][seed_phi1+3];
        	tmp3 		 = temp[seed_eta1+m][seed_phi1+4] + tmp1;
...
// now we are in the left bottom corner
        ...
        for(loop m=0; m<5 ; m++){
        	#pragma HLS UNROLL
        	tmp1	 		= temp[seed_eta1+m][seed_phi1]   + temp[seed_eta1+m][seed_phi1+1];
        	tmp2 			= temp[seed_eta1+m][seed_phi1+2] + temp[seed_eta1+m][seed_phi1+3];
            tmp3 			= temp[seed_eta1+m][seed_phi1+4] + tmp1;
            eta_slice[m] 	= tmp2 + tmp3;
			}
          ...
          etSum2x5 = (et2x5_1Tot >= et2x5_2Tot)?et2x5_1Tot:et2x5_2Tot;
...
//finding the the cluster stitch condition in the upper two 3x4 region
...
for(loop i=0; i<4; i++){
    for(loop j=4; j<8; j++){
        ...
        if(stitch == 1){
            ...
            ap_uint<17> et5x5           = ClusterIn[i].et5x5() + ClusterIn[j].et5x5();
            ap_uint<15> pegged_et5x5    = (et5x5 > 0x7FFF) ? (ap_uint<15>)0x7FFF : (ap_uint<15>) et5x5;

            ap_uint<17> et2x5           = ClusterIn[i].et2x5() + ClusterIn[j].et2x5();
            ap_uint<15> pegged_et2x5    = (et2x5 > 0x7FFF) ? (ap_uint<15>)0x7FFF : (ap_uint<15>) et2x5;

            if(ClusterIn[i].clusterEnergy() > ClusterIn[j].clusterEnergy()){

                ClusterOut[i] = Cluster(pegged_cEtSum, ClusterIn[i].towerEta(), ClusterIn[i].towerPhi(), ClusterIn[i].clusterEta(), ClusterIn[i].clusterPhi(), ClusterIn[i].satur(), pegged_et5x5, pegged_et2x5, ClusterIn[i].brems());
                ClusterOut[j] = Cluster(0, 0, 0, 0, 0, 0, 0, 0, 0);
            }
            else{
                ClusterOut[i] = Cluster(0, 0, 0, 0, 0, 0, 0, 0, 0);
                ClusterOut[j] = Cluster(pegged_cEtSum, ClusterIn[i].towerEta(), ClusterIn[i].towerPhi(), ClusterIn[i].clusterEta(), ClusterIn[i].clusterPhi(), ClusterIn[i].satur(), pegged_et5x5, pegged_et2x5, ClusterIn[i].brems());
            }
        }
    }
}

//finding the the cluster stitch condition in the lower two 3x4 region
